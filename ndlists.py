...
class ndlist(list):
    ...
    def __setitem__(self, key, value):  # DOES NOT WORK, even vaguely !!!!!!
        if isinstance(key, tuple) and len(key) > self.ndim:
            raise IndexError(f"Too many indices: object is {self.ndim}-dimensional, index is {len(key)}-dimensional ")
        key = ndslicenormalize(key)
        keyshape = [self._slicelen(k, sh) for k, sh in zip(key, self.shape)]
        #       Add error checks in keyshape?
        #        print(keyshape)
        match value:
            case numbers.Number():  # print("Loop overall positions")
                for s in self.ndsliceiteration(key):
                    settupleindex(self._lst, s, value)  #  Do lst[s0][s1]...[slast]=value
            case collections.abc.Iterable():
                # check shape before assigning value?
                for s, sval in zip(self.ndsliceiteration(key),
                                   itertools.product(*[range(k) for k in keyshape if k > 1])
                                   ):
                    if len(sval) == 1:
                        sval = sval[0]  # TODO optimize if inside the loop away using keyshape
                    settupleindex(self._lst, s, value[sval])
            case _:
                raise NotImplementedError(f"Assignment of {type(value)} not possible yet")
